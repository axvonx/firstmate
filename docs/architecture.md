...
Which rung a wake carries is derived from that completed-turn age itself rather than from a re-notice timer, and the marker recording the highest rung already surfaced is reset only by a completed turn, so the ladder's position cannot drift from the age that justifies it.
