...
FM_BROWSER_SWEEP_INTERVAL=900   # seconds between watcher sweeps of orphaned per-task browser sessions
FM_PROCEVENT_MAX_OUTPUT_BYTES=1048576   # bound on one captured process-to-event result
FM_PROCEVENT_CLAIM_ROOT=                # machine-wide source claim root; default $XDG_STATE_HOME/firstmate/procevent-claims
FM_CODEX_WATCH_CHECKPOINT=180   # seconds per foreground watcher checkpoint in Codex primary supervision
FM_CREW_STATE_NM_TIMEOUT=10   # seconds allowed per no-mistakes query inside fm-crew-state.sh
FM_CREW_STATE_RUNS_LIMIT=200  # recent no-mistakes run rows scanned when axi status cannot be attributed to the current code
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the current-state reader used by working/paused watcher triage and by the step-progress recheck both supervisors run before a wedge escalation
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane is RECHECKED as a possible wedge; it escalates only if that recheck cannot show the run advancing. Stale panes whose crew is not provably working surface immediately unless they declare the pause verb
FM_STEP_ACTIVITY_FRESH_SECS=1800   # how recently the run's active pipeline step must have logged SOMETHING NEW for that alone to count as advancing; a repeated identical log line is recency without change, so it does not count unless the step reports it has no agent that could loop (step-agent: none, as a ci monitor does), where recency alone counts
FM_STEP_STALL_MAX_SECS=7200        # ceiling on how long a step may log nothing new while its agent process is still alive before it escalates anyway; a live agent is evidence of work, not proof of progress
FM_STEP_PROGRESS_SURFACE_COUNT=15  # confirmed-progress rechecks one pane may absorb before one LONG-RUNNING notice surfaces and the count restarts; at the 240s recheck bound that is a first glance at roughly an hour, then roughly hourly, and it is explicitly not a wedge report
FM_BUSY_TURN_MAX_SECS=3600         # maximum age of a busy pane's latest state/<id>.turn-ended marker, or its state/<id>.meta spawn record before any turn completes, before the busy pane's own ladder takes over; inspection-only, never an automatic interrupt or restart
FM_BUSY_TURN_RENOTICE_SECS=        # unset/blank defaults to FM_BUSY_TURN_MAX_SECS; how often a busy pane past that bound surfaces again while the same turn stays open. A declared pause or captain hold takes the bounded pause cadence instead, the first window and every advancing window surface a LONG-RUNNING notice that is explicitly not a wedge report, and only the windows after the first escalate as a possible wedge with the escalation count and demand-deep-inspection marker. The bound crossing itself counts as that first window, so at the defaults (both 3600s) the ladder is a notice at a completed-turn age of 1h, escalation 1 at 2h, escalation 2 at 3h, and escalation 3 plus demand-deep-inspection at 4h. The rung is recomputed from the completed-turn age on every poll rather than tracked by a timer, so a change to this value applies to a turn already in flight instead of only from the next window
...
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # escalations on the same unchanged pane where the run reported no advancing step, before demand-deep-inspection is added; an intervening confirmed-advancing absorb does not clear the count
