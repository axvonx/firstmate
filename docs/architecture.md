...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` and whose run cannot be shown to be advancing, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
Reaching `FM_STALE_ESCALATE_SECS` does not escalate on its own, because a running pipeline step legitimately holds a silent pane for its whole 10-55 minute length.
The watcher first asks the run whether it is advancing, through `crew_step_is_advancing`, and escalates only when that check cannot find progress; a confirmed-advancing run restarts the timer instead.
Advancing means the step logged something NEW within `FM_STEP_ACTIVITY_FRESH_SECS`, or its step agent is still alive within `FM_STEP_STALL_MAX_SECS`, so a step that keeps re-logging one identical line is not mistaken for one that is getting somewhere.
The changed-text requirement applies only where a subprocess agent could loop: a step that reports `step-agent: none`, such as the ci monitor whose progress markers repeat verbatim, is read on log recency alone and still escalates once that recency lapses.
That licence needs the explicit reading; a step-agent field that is absent because the pid could not be read is an uncertain reading and escalates, so no rendering change can quietly move every agent-run step onto the recency-only path.
Escalations on the same unchanged pane add an escalation count to the wake reason and, at `FM_WEDGE_DEMAND_INSPECT_COUNT`, a `demand-deep-inspection` marker, which means the cheap progress check has been exhausted rather than merely that the pane has been quiet.
That count survives an intervening confirmed-advancing absorb, so a lane that alternates between looking busy and showing no progress still reaches the marker.
Absorbing is bounded in the other direction too: every `FM_STEP_PROGRESS_SURFACE_COUNT` confirmed-progress absorbs on one pane surface a single long-running notice, worded explicitly as not a wedge, and then the count restarts on the same long cadence.
A busy pane is otherwise exempt from staleness, but only until its latest `state/<id>.turn-ended` marker reaches `FM_BUSY_TURN_MAX_SECS`, or its `state/<id>.meta` spawn record reaches that age before any turn completes.
Past that bound the pane gets its own ladder rather than the pipeline-step one above, because absence of step progress is not evidence about a worker whose harness is authoritatively busy and may have no run open at all.
A declared `paused:` or durable `captain-held` line takes the bounded pause cadence there even though the busy pane still outranks it as current state, so a worker on a known external wait is not alarmed on merely for running a command.
Otherwise the pane surfaces one wake per `FM_BUSY_TURN_RENOTICE_SECS`, starting with the bound crossing itself: an advancing run and that first window each surface a long-running notice worded explicitly as not a wedge, and only a turn still uncompleted a whole further window later escalates as a possible wedge, carrying the escalation count and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
At the defaults, where the bound and the window are both an hour, that is a notice at a completed-turn age of 1h, escalation 1 at 2h, escalation 2 at 3h, and escalation 3 plus `demand-deep-inspection` at 4h.
Which rung a wake carries is derived from that completed-turn age itself rather than from a re-notice timer, and the marker recording the highest rung already surfaced is keyed on the turn it was surfaced against, so a completed turn retires that record with no reset step anywhere and the ladder's position cannot drift from the age that justifies it.
A hung foreground call is therefore still detected and still escalates without limit, named a wedge one window after the bound rather than four minutes past it.
...
A crew that declares `paused:` for a known external wait is separately absorbed and re-surfaced only on the longer pause cadence, rather than being treated as a possible wedge.
That absorption reaches the declaring crew whether its pane is idle or busy past the completed-turn bound, so the declaration is a remedy the worker can actually reach; it decides only whether the idling is suspicious, and never becomes the crew's reported current state while an authoritative busy or run-step signal says otherwise.
...
A crew's own `done: PR <url> checks green` status line is a claim about CI rather than a reading of it, so it never surfaces the PR as ready on its own while an attributed run is still working.
[`verification/ci-checks-green.md`](verification/ci-checks-green.md) owns the marker evidence, the residual no-run-attributed case, and the one false-green vector this layer cannot detect.
...
In that status-log fallback, a declared external wait and a durable `captain-held` transfer both report the distinct `paused` state with the declaration's own note as the detail, so a held crew keeps both the hold and its reason instead of falling through to unknown.
...
The arm layer records one bounded lifecycle row per observed cycle in `state/.watch-cycle-exits.log`; `state/.watch-triage.log` remains the watcher's own bounded debug log and carries no lifecycle rows.
