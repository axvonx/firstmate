...
A secondmate's crewmates read worker credentials from the primary home's `.env` when the secondmate home has none that delivers, and that fallback lives in `bin/fm-spawn.sh`, which each secondmate runs from its own checkout - so it reaches that lane only once this same guarded tracked-file fast-forward has landed the version carrying it.
Confirm every registered secondmate home carries it before clearing any machine-wide credential copies, including a lane whose `home=` resolves from `data/secondmates.md` rather than from its meta record; a lane that resolves from neither is a stop, never a silent skip.
[`docs/configuration.md`](../../../docs/configuration.md) owns the mechanism, the per-home check that answers out loud for every lane, and the confirmation signal.
