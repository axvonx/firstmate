...
FM_STEP_ACTIVITY_FRESH_SECS=1800   # how recently the run's active pipeline step must have logged for that alone to count as advancing
FM_STEP_STALL_MAX_SECS=7200        # ceiling on how long a step may log nothing while its agent process is still alive before it escalates anyway; a live agent is evidence of work, not proof of progress
...
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive escalations on the same unchanged pane where the run reported no advancing step, before demand-deep-inspection is added
