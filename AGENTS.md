...
.env                 this home's worker credentials, plus the optional X-mode pairing token; LOCAL, gitignored, never committed; loaded into every worker's environment at spawn EXCEPT its FM_*/FMX_* names, which stay with the home (bin/fm-worker-env-lib.sh); presence-gates section 14
