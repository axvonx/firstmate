...
.env                 this home's worker credentials, plus the optional X-mode pairing token; LOCAL, gitignored, never committed; loaded into every worker's environment at spawn EXCEPT its FM_*/FMX_* names and names that would redirect what a worker executes, which stay with the home (bin/fm-worker-env-lib.sh); a home without one reads its primary's, announced at spawn, and its own file wins whenever it exists; presence-gates section 14
