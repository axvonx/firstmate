...
The helper requires a full `https://github.com/<owner>/<repo>/pull/<n>` URL, invokes `gh-axi pr merge <n> --repo <owner>/<repo>`, defaults to `--squash` and `--delete-branch` (the forge's own `delete_branch_on_merge` deletes only head branches that live in the base repository, so a fork-side head branch needs the merge to ask), preserves explicit merge-method flags and a caller's own `--delete-branch`, and rejects malformed URLs or repo override flags before recording merge state; a well-formed GitLab merge request URL (see [docs/gitlab-merge-watch.md](gitlab-merge-watch.md)) is refused too, explicitly, rather than sent to the wrong forge.
