...
The changed-text requirement applies only where a subprocess agent could loop: a step that publishes no step agent at all, such as the ci monitor whose progress markers repeat verbatim, is read on log recency alone and still escalates once that recency lapses.
