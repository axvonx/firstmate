...
3. Use gh-axi for GitHub operations and chrome-devtools-axi for browser operations.
   When browser work is finished, run \`chrome-devtools-axi open about:blank\`, then \`chrome-devtools-axi stop\`; there is no \`close\` command, and \`closepage\` leaves Chrome running.
   Park first because it is the half that survives failure: an abandoned page keeps rendering at full CPU indefinitely, and parking still holds if the stop is missed or the task ends mid-run.
