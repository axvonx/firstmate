...
Repeated escalations on the same unchanged pane add an escalation count to the wake reason and, at `FM_WEDGE_DEMAND_INSPECT_COUNT`, a `demand-deep-inspection` marker, which means the cheap progress check has been exhausted rather than merely that the pane has been quiet.
...
For a still-working run it additionally publishes `activity: <N>s` and `step-agent: alive|gone`, the two readings supervision uses to tell an advancing step from a frozen one.
The script header owns the exact run-head ancestry rules, the pipeline-owned conditions, and why neither progress reading is sufficient alone.
