...
family_for_basename() {
  case "$1" in
    ...
    fm-brief.test.sh|fm-browser-lib.test.sh|fm-vendor-auth-probe.test.sh|\
    ...
    fm-teardown-endpoint-safety.test.sh)
      printf '%s\n' backend-dispatch
      ;;
    fm-pr-check-security.test.sh|fm-pr-merge.test.sh|fm-project-branch-cleanup.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-backend.sh|bin/fm-backend-hometag-lib.sh)
      ...
      printf '%s\n' real-herdr-gated
      ;;
    bin/fm-browser-lib.sh)
      # Its own unit suite, plus the brief, teardown, watcher, and spawn suites
      # that cover the four places per-task browser sessions are wired in.
      printf '%s\n' pure-contract-unit
      printf '%s\n' pr-forge
      printf '%s\n' watcher-wake-lock
      printf '%s\n' backend-dispatch
    ...
    bin/fm-gate-refuse*|bin/fm-lock*|bin/fm-quota-axi-lib.sh)
      printf '%s\n' session-bootstrap
      ;;
    bin/fm-pr-*|bin/fm-merge-local.sh|bin/fm-project-branch-cleanup.sh|\
    bin/fm-teardown.sh|bin/fm-review-diff.sh|\
