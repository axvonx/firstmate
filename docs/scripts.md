...
The table is a curated list of what a reader is likely to reach for rather than an exhaustive inventory, so treat `bin/` itself as authoritative for what exists.
...
| `fm-ci-run-serial-lane.sh` | Run one portable serial CI half under an explicit time budget so an overrun reports itself instead of cancelling the job |
| `fm-test-timing-aggregate.sh` | Aggregate CI lane timing JSON, accounting for every expected lane by name so a dropped lane fails instead of shrinking the summary |
...
| `fm-project-branch-cleanup.sh` | Verify, and set when needed, a project's forge delete-branch-on-merge setting, reporting a blocker instead of failing |
...
| `fm-teardown.sh`         | Fail-closed teardown: prove worktree ownership, return landed ship worktrees, require completed scout deliverables, retire secondmate homes |
| `fm-worktree-owner-lib.sh` | Own the worktree ownership record proving a task still occupies the worktree its metadata names |
| `fm-worktree-claim.sh`   | Re-establish a task's worktree ownership record after a verified operator check |
