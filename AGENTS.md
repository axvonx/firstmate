...
.env                 this home's worker credentials, plus the optional X-mode pairing token; LOCAL, gitignored, never committed; loaded into every worker's environment at spawn EXCEPT its FM_*/FMX_* names and names that would redirect what a worker executes, which stay with the home (bin/fm-worker-env-lib.sh); a home whose own file delivers no worker credential reads its primary's, announced at spawn, and its own file wins whenever it actually delivers one; a non-empty FMX_PAIRING_TOKEN in it gates section 14, never the file's presence, so a credentials-only .env leaves X mode off
...
config/vault-only-keys  the credential names this home keeps in Automic Vault, one per line; LOCAL, gitignored; absent = vault discipline applies to every credential, present = it applies to exactly those names while everything else is read from the environment; empty or malformed refuses rather than narrowing silently; inherited by secondmate homes (bin/fm-brief.sh --help)
...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: window=, endpoint_task_id=, worktree=, worktree_token=, project=, harness=, model=, effort=, kind=, mode=, yolo=, tasktmp=, browser_session=; kind=secondmate also records home= and projects=; fm-teardown records worktree_returned= once the task's pooled worktree has gone back to the pool, so no rerun returns or resets that path again; a non-default runtime backend records further backend-specific fields (docs/configuration.md "Runtime backend"; bin/fm-backend.sh, section 8); fm-pr-check, including through fm-pr-merge, records one canonical pr= and the forge's pr_head= when available (GitHub pull requests and GitLab merge requests; docs/gitlab-merge-watch.md); fm-x-link appends x_request=, x_request_ts=, x_followups=, and optional x_platform=/x_reply_max_chars= for an X-mode-originated task (section 14)
  ...
  .hash-* .count-* .stale-* .stale-since-* .paused-* .wedge-escalations-* .step-activity-* .step-progress-* .busy-turn-rung-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
...
Firstmate's own browser work shares the default session with the captain and other tools unless this home was itself given a private one, so park each page you opened with `chrome-devtools-axi open about:blank` when you are done, never park a page you did not open, and never `stop` a session you did not create; each task's worker instead gets a private session that firstmate creates and reclaims for it (`bin/fm-browser-lib.sh`).
Never print a credential value, and never print it indirectly either through an environment dump, shell tracing, a verbose authenticated request, a whole-object debug print, a raw crash trace, or a fixture built from live values; where a credential vault is installed reach the keys that home declares vault-only through that verified vault, never answering a vault failure by rerunning the command bare, and read every other credential from the environment without a vault call.
`bin/fm-brief.sh`'s generated credential rule and its `--help` own the never-print discipline, the vault-only key list, the identity probe, the invocation form, the stop-and-report routing, and the reasoning, for firstmate and secondmates as much as for the crewmates it briefs.
...
For a no-mistakes ship, the worker carries its own implementation commit straight into validation, so firstmate neither triggers it nor waits for a report at that point; steering a worker that stopped there anyway is the one remaining use of the harness invocation owned by `harness-adapters`.
Nothing is guaranteed to interrupt between the commit and the run, so a requirement known at intake belongs in the brief rather than saved for that moment.
...
A teardown refusal is a stop-and-investigate result, never an obstacle to bypass, whether it names uncommitted or unlanded work or reports that teardown cannot prove the worktree is still this task's.
Never force teardown without explicit discard authority; `--force` never waives an ownership refusal, which clears only through the exit that refusal names, after checking by hand that the worktree still holds this task's agent and work.
