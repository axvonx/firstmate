...
A secondmate's crewmates read worker credentials from the primary home's `.env` when the secondmate home has none that delivers, and the secondmate finds that primary only through the pointer on its own launch command, so a secondmate agent launched before that fallback existed leaves its crewmates with no credential source and must be respawned to gain it.
Respawn every registered secondmate before clearing any machine-wide credential copies, and read [`docs/configuration.md`](../../../docs/configuration.md) for the mechanism, the listing check, and how to confirm the pointer arrived.
