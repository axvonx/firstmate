...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: window=, endpoint_task_id=, worktree=, worktree_token=, project=, harness=, model=, effort=, kind=, mode=, yolo=, tasktmp=, browser_session=; kind=secondmate also records home= and projects=; fm-teardown records worktree_returned= once the task's pooled worktree has gone back to the pool, so no rerun touches that path again; a non-default runtime backend records further backend-specific fields (docs/configuration.md "Runtime backend"; bin/fm-backend.sh, section 8); fm-pr-check, including through fm-pr-merge, records one canonical pr= and the forge's pr_head= when available (GitHub pull requests and GitLab merge requests; docs/gitlab-merge-watch.md); fm-x-link appends x_request=, x_request_ts=, x_followups=, and optional x_platform=/x_reply_max_chars= for an X-mode-originated task (section 14)
