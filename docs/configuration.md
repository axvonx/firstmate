...
FM_STEP_ACTIVITY_FRESH_SECS=1800   # how recently the run's active pipeline step must have logged SOMETHING NEW for that alone to count as advancing; a repeated identical log line is recency without change, so it does not count
